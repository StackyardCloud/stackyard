...
const searchManagementQueryKeysAPIVersion = "2025-05-01"

type searchManagementQueryKeysClient struct {
	endpoint string
...
func newSearchManagementQueryKeysClient(endpoint, account, subscriptionKey string) *searchManagementQueryKeysClient {
	pipeline := runtime.NewPipeline(
		"stackyard",
		"search-management-resource-manager-query-keys-2025-05-01",
		runtime.PipelineOptions{
			PerRetry: []policy.Policy{&sharedKeyAndSubscriptionPolicy{account: account, subscriptionKey: subscriptionKey}},
		},
		&policy.ClientOptions{},
	)
	return &searchManagementQueryKeysClient{
		endpoint: strings.TrimRight(endpoint, "/"),
		pipeline: pipeline,
	}
}

func (c *searchManagementQueryKeysClient) doJSON(ctx context.Context, method, path string, payload any, expectedStatuses ...int) (map[string]any, int, error) {
...
func main() {
	...
	queryKeyName := getenv("STACKYARD_AZURE_QUERY_KEY_NAME", "my-query-key")
	queryKeyValue := getenv("STACKYARD_AZURE_QUERY_KEY_VALUE", "querykey123")

	fmt.Printf("Stackyard Azure Search Management - Resource Manager - Query Keys (search-management-resource-manager-query-keys-2025-05-01) example using %s\n", strings.TrimRight(endpoint, "/"))

	client := newSearchManagementQueryKeysClient(endpoint, account, subscriptionKey)
	...
	calls := []struct {
	...
	}{
		{
			...
			path:     basePath + "/createQueryKey/" + queryKeyName,
			statuses: []int{http.StatusOK, http.StatusNotImplemented},
		},
		{
			name:     "ListQueryKeys",
			method:   http.MethodPost,
			path:     basePath + "/listQueryKeys",
			statuses: []int{http.StatusOK, http.StatusNotImplemented},
		},
		{
			name:     "DeleteQueryKey",
			method:   http.MethodDelete,
			path:     basePath + "/deleteQueryKey/" + queryKeyValue,
			statuses: []int{http.StatusOK, http.StatusNoContent, http.StatusNotFound, http.StatusNotImplemented},
		},
	}

	notImplementedCount := 0
	for _, call := range calls {
		fullPath := withAPIVersion(call.path, searchManagementQueryKeysAPIVersion)
	...
	if notImplementedCount == len(calls) {
		fmt.Println("All search-management resource-manager query-keys routes are staged in this Stackyard build.")
		return
	}
	fmt.Println("Done.")
}
