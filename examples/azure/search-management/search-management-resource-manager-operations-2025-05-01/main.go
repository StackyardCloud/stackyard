...
const searchManagementOperationsAPIVersion = "2025-05-01"

type searchManagementOperationsClient struct {
	endpoint string
...
func newSearchManagementOperationsClient(endpoint, account, subscriptionKey string) *searchManagementOperationsClient {
	pipeline := runtime.NewPipeline(
		"stackyard",
		"search-management-resource-manager-operations-2025-05-01",
		runtime.PipelineOptions{
			PerRetry: []policy.Policy{&sharedKeyAndSubscriptionPolicy{account: account, subscriptionKey: subscriptionKey}},
		},
		&policy.ClientOptions{},
	)
	return &searchManagementOperationsClient{
		endpoint: strings.TrimRight(endpoint, "/"),
		pipeline: pipeline,
	}
}

func (c *searchManagementOperationsClient) doJSON(ctx context.Context, method, path string, payload any, expectedStatuses ...int) (map[string]any, int, error) {
...
func main() {
	...
	fmt.Printf("Stackyard Azure Search Management - Resource Manager - Operations (search-management-resource-manager-operations-2025-05-01) example using %s\n", strings.TrimRight(endpoint, "/"))

	client := newSearchManagementOperationsClient(endpoint, account, subscriptionKey)
	path := withAPIVersion("/azure/providers/Microsoft.Search/operations", searchManagementOperationsAPIVersion)
	_, status, err := client.doJSON(ctx, http.MethodGet, path, nil, http.StatusOK, http.StatusNotImplemented)
	if err != nil {
		exitf("OperationsList failed: %v", err)
	}
	if status == http.StatusNotImplemented {
		notImplemented(path)
		fmt.Println("All search-management resource-manager operations routes are staged in this Stackyard build.")
		return
	}
	fmt.Printf("OperationsList: status=%d\n", status)
	fmt.Println("Done.")
}
