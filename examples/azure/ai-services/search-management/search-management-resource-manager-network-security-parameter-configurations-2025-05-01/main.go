...
const searchManagementNetworkSecurityPerimeterConfigurationsAPIVersion = "2025-05-01"

type searchManagementNetworkSecurityPerimeterConfigurationsClient struct {
	endpoint string
...
func newSearchManagementNetworkSecurityPerimeterConfigurationsClient(endpoint, account, subscriptionKey string) *searchManagementNetworkSecurityPerimeterConfigurationsClient {
	pipeline := runtime.NewPipeline(
		"stackyard",
		"search-management-resource-manager-network-security-parameter-configurations-2025-05-01",
		runtime.PipelineOptions{
			PerRetry: []policy.Policy{&sharedKeyAndSubscriptionPolicy{account: account, subscriptionKey: subscriptionKey}},
		},
		&policy.ClientOptions{},
	)
	return &searchManagementNetworkSecurityPerimeterConfigurationsClient{
		endpoint: strings.TrimRight(endpoint, "/"),
		pipeline: pipeline,
	}
}

func (c *searchManagementNetworkSecurityPerimeterConfigurationsClient) doJSON(ctx context.Context, method, path string, payload any, expectedStatuses ...int) (map[string]any, int, error) {
...
func main() {
	...
	nspConfigName := getenv("STACKYARD_AZURE_SEARCH_NSP_CONFIG_NAME", "default")

	fmt.Printf("Stackyard Azure Search Management - Resource Manager - Network Security Perimeter Configurations (search-management-resource-manager-network-security-parameter-configurations-2025-05-01) example using %s\n", strings.TrimRight(endpoint, "/"))

	client := newSearchManagementNetworkSecurityPerimeterConfigurationsClient(endpoint, account, subscriptionKey)
	collectionPath := "/azure/subscriptions/" + subscriptionID +
		"/resourceGroups/" + resourceGroup +
		"/providers/Microsoft.Search/searchServices/" + searchServiceName +
		"/networkSecurityPerimeterConfigurations"
	entityPath := collectionPath + "/" + nspConfigName
	...
	for _, call := range calls {
		fullPath := withAPIVersion(call.path, searchManagementNetworkSecurityPerimeterConfigurationsAPIVersion)
	...
	if notImplementedCount == len(calls) {
		fmt.Println("All search-management resource-manager network-security-perimeter-configurations routes are staged in this Stackyard build.")
		return
	}
	fmt.Println("Done.")
}
