...
const searchServiceSkillsetsAPIVersion = "2025-09-01"

type searchServiceSkillsetsClient struct {
	endpoint string
...
func newSearchServiceSkillsetsClient(endpoint, account, subscriptionKey string) *searchServiceSkillsetsClient {
	pipeline := runtime.NewPipeline(
		"stackyard",
		"search-service-data-plane-skillsets-2025-09-01",
		runtime.PipelineOptions{
			PerRetry: []policy.Policy{&sharedKeyAndSubscriptionPolicy{account: account, subscriptionKey: subscriptionKey}},
		},
		&policy.ClientOptions{},
	)
	return &searchServiceSkillsetsClient{
		endpoint: strings.TrimRight(endpoint, "/"),
		pipeline: pipeline,
	}
}

func (c *searchServiceSkillsetsClient) doJSON(ctx context.Context, method, path string, payload any, extraHeaders map[string]string, expectedStatuses ...int) (map[string]any, int, error) {
...
func main() {
	...
	skillsetName := getenv("STACKYARD_AZURE_SEARCH_SKILLSET", "hotels-skillset")

	fmt.Printf("Stackyard Azure Search Service - Data Plane - Skillsets (search-service-data-plane-skillsets-2025-09-01) example using %s\n", strings.TrimRight(endpoint, "/"))

	client := newSearchServiceSkillsetsClient(endpoint, account, subscriptionKey)
	collectionPath := "/azure/skillsets"
	entityPath := "/azure/skillsets('" + skillsetName + "')"

	skillsetPayload := map[string]any{
		"name": skillsetName,
	...
	for _, call := range calls {
		fullPath := withAPIVersion(call.path, searchServiceSkillsetsAPIVersion)
	...
	if notImplementedCount == len(calls) {
		fmt.Println("All search-service data-plane skillsets routes are staged in this Stackyard build.")
		return
	}
	fmt.Println("Done.")
}
