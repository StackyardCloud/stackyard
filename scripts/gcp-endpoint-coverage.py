#!/usr/bin/env python3
"""
GCP endpoint coverage aggregator for Stackyard.

This script aggregates GCP contract/io/doc coverage signals into a single
...
CONTRACT_SCRIPT = SCRIPT_DIR / "gcp-contract-coverage.py"
IO_SCRIPT = SCRIPT_DIR / "gcp-io-contract-coverage.py"
DOC_SCRIPT = SCRIPT_DIR / "gcp-doc-contract-coverage.py"
...
def _gcp_list_fragment_validator(expected_fragment: str) -> callable:
    def validate(status: int, _headers: dict[str, str], body: bytes) -> tuple[bool, str]:
        if status != 200:
            return False, f"expected 200, got {status}"
        rendered = body.decode("utf-8")
        if expected_fragment not in rendered:
            return False, f"expected {expected_fragment!r} in response body"
        return True, f"response contained {expected_fragment!r}"

    return validate


def _gcp_invalid_argument_validator(status: int, _headers: dict[str, str], body: bytes) -> tuple[bool, str]:
    if status != 400:
        return False, f"expected 400, got {status}"
    payload = _json_payload(body)
    if payload.get("error") != "InvalidArgument":
        return False, f"expected InvalidArgument, got {payload!r}"
    return True, "route returned InvalidArgument"


def _cloudprofiler_create_validator(status: int, _headers: dict[str, str], body: bytes) -> tuple[bool, str]:
    if status != 200:
        return False, f"expected 200, got {status}"
    payload = _json_payload(body)
    deployment = payload.get("deployment")
    if not isinstance(deployment, dict) or deployment.get("target") != "stackyard-service":
        return False, f"unexpected deployment payload: {payload!r}"
    return True, "cloudprofiler create profile returned deployment payload"


def _apigeeconnect_egress_validator(status: int, _headers: dict[str, str], body: bytes) -> tuple[bool, str]:
    if status != 200:
        return False, f"expected 200, got {status}"
    payload = _json_payload(body)
    http_response = payload.get("httpResponse")
    if not isinstance(http_response, dict) or http_response.get("status") != 200:
        return False, f"unexpected egress payload: {payload!r}"
    return True, "apigeeconnect egress returned httpResponse payload"


GCP_LIVE_SMOKE_CHECKS: tuple[HTTPCheck, ...] = (
    HTTPCheck(
        service="cloudprofiler",
        name="list_profiles",
        method="GET",
        path="/gcp/v2/projects/stackyard/profiles?pageSize=1",
        validator=_gcp_list_fragment_validator("stackyard-profile"),
    ),
    HTTPCheck(
        service="cloudprofiler",
        name="create_profile",
        method="POST",
        path="/gcp/v2/projects/stackyard/profiles",
        headers={"Content-Type": "application/json"},
        body=b'{"deployment":{"projectId":"stackyard","target":"stackyard-service"},"profileType":["CPU"]}',
        validator=_cloudprofiler_create_validator,
    ),
    HTTPCheck(
        service="cloudprofiler",
        name="invalid_page_size",
        method="GET",
        path="/gcp/v2/projects/stackyard/profiles?pageSize=bad",
        validator=_gcp_invalid_argument_validator,
    ),
    HTTPCheck(
        service="apigeeconnect",
        name="list_connections",
        method="GET",
        path="/gcp/v1/projects/stackyard/endpoints/local/connections?pageSize=1",
        validator=_gcp_list_fragment_validator("connections/conn-1"),
    ),
    HTTPCheck(
        service="apigeeconnect",
        name="egress",
        method="POST",
        path="/gcp/v1/projects/stackyard/endpoints/local:egress",
        headers={"Content-Type": "application/json"},
        body=b'{"id":"egress-bootstrap"}',
        validator=_apigeeconnect_egress_validator,
    ),
    HTTPCheck(
        service="apigeeconnect",
        name="invalid_page_size",
        method="GET",
        path="/gcp/v1/projects/stackyard/endpoints/local/connections?pageSize=bad",
        validator=_gcp_invalid_argument_validator,
    ),
)
...
def main() -> int:
    ...
    if args.live_smoke:
        selected_checks = [check for check in GCP_LIVE_SMOKE_CHECKS if check.service in services]
    ...
    try:
        ...
        if args.format == "json":
            ...
            print("GCP endpoint coverage (aggregated)")
