#!/usr/bin/env python3
"""
GCP provider documentation-backed contract coverage analyzer for Stackyard.

This analyzer combines documentation-source checks with implementation/test
contract signals to provide parity with Azure/AWS doc contract tooling.
"""
...
ALLOWED_DOC_HOST_MARKERS = (
    ...
    "raw.githubusercontent.com/googleapis/",
    "firebase.google.com",
    "ai.google.dev",
)
...
def discover_service_files() -> dict[str, list[Path]]:
    ...
    for path in SERVER_DIR.glob("provider_gcp_*.go"):
        if path.name.endswith("_test.go"):
            continue
        service = path.stem.removeprefix("provider_gcp_")
...
def discover_test_files(service: str) -> list[Path]:
    return sorted(SERVER_DIR.glob(f"provider_gcp_{service}*_test.go"))
...
def matching_plan_docs(service: str) -> list[Path]:
    ...
    out: list[Path] = []
    for path in sorted(DOCS_DIR.glob("gcp-*-plan.md")):
...
def main() -> int:
    ...
    selected = sorted(service for service in files_by_service if fnmatch.fnmatch(service, selector))
    if required and required not in files_by_service:
        print(f"gcp-doc-contract-coverage: required service {required!r} not discovered", file=sys.stderr)
        return 2
    if required and required not in selected:
        print(f"gcp-doc-contract-coverage: required service {required!r} not matched by selector", file=sys.stderr)
        return 2
    ...
    if should_fail(rows, parse_fail_on(args.fail_on)):
        ...
        if failing:
            print("gcp-doc-contract-coverage: failing services: " + ", ".join(failing), file=sys.stderr)
        return 1
    return 0
