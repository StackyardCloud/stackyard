#!/usr/bin/env python3
"""
AWS provider contract coverage wrapper for Stackyard.

This wrapper aligns the AWS contract coverage invocation style with the
provider-scoped GCP/Azure coverage scripts while delegating execution to
scripts/awscli-endpoint-coverage.py.
"""
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(description="Analyze AWS provider contract coverage.")
    parser.add_argument(
        "--service",
        default="*",
...
def ensure_required_service(args: argparse.Namespace) -> int:
    ...
    if cp.returncode != 0:
        print(
            f"aws-contract-coverage: failed to validate required service {required_service!r}",
            file=sys.stderr,
        )
        if combined.strip():
    ...
    if selected is None:
        print(
            "aws-contract-coverage: unable to determine selected endpoint count while checking "
    ...
    if selected == 0:
        print(
            f"aws-contract-coverage: required service {required_service!r} not found",
            file=sys.stderr,
        )
        return 2
    return 0
...
def parse_fail_on(raw: str) -> str:
    ...
    for item in raw.split(","):
        ...
        if token in {"any", "strict"}:
            for status in DEFAULT_FAIL_ON.split(","):
                status = status.strip()
...
def main() -> int:
    ...
    if args.report_json:
        ...
        tmp = tempfile.NamedTemporaryFile(
            prefix="stackyard-aws-contract-coverage-",
            suffix=".json",
            delete=False,
        )
        tmp.close()
