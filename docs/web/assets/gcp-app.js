...
function matchesQuery(service, query, categories) {
  if (!query) {
    return true;
  }
...
function renderFilterChips(services, activeCategory, categories, categoryOrder) {
...
function renderCatalog(services, state, categories, categoryOrder) {
  ...
  const filtered = services
    .filter((service) => matchesQuery(service, state.query, categories))
...
function setupGCPCatalog() {
  ...
  renderStats(catalog);
  renderSidebarNav(services);
  renderFilterChips(services, state.category, categories, categoryOrder);
  renderCatalog(services, state, categories, categoryOrder);
