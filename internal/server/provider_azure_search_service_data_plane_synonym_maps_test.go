...
func TestAzureSearchServiceDataPlaneSynonymMapsRoutesReturnNotImplemented(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			path:   "/azure/synonymmaps?api-version=2025-09-01",
			body:   []byte(`{"name":"hotels-synonyms","format":"solr","synonyms":"usa, united states\nhotel, lodging"}`),
		},
		{
			name:   "create or update",
			method: http.MethodPut,
			path:   "/azure/synonymmaps('hotels-synonyms')?api-version=2025-09-01",
			body:   []byte(`{"name":"hotels-synonyms","format":"solr","synonyms":"usa, united states\nhotel, lodging"}`),
		},
		{
			name:   "delete",
		...
		},
		{
			name:   "list",
			method: http.MethodGet,
			path:   "/azure/synonymmaps?api-version=2025-09-01&$select=name",
		},
	}
...
func TestAzureSearchServiceDataPlaneSynonymMapsUnsupportedNestedPathReturnsNotImplemented(t *testing.T) {
	t.Parallel()
	...
	path := "/azure/synonymmaps('hotels-synonyms')/unsupported/segment"
	resp := providerContractRequest(t, ts, http.MethodGet, path, nil, map[string]string{
		"Authorization": "SharedKey devstoreaccount1:signature",
	})
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 for unknown synonym maps nested path, got %d body=%s", resp.StatusCode, string(providerContractBody(t, resp)))
	}
	payload := providerContractJSONMap(t, resp)
