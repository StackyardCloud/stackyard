...
func (s *Server) handleAzureAIFoundryAccountManagementRouter(w http.ResponseWriter, r *http.Request) bool {
	...
	if !azureIsAIFoundryAccountManagementPath(segments) {
		return false
	}
	...
	if !azureAIFoundryAccountManagementAllowedMethod(r.Method) {
		return false
	}
...
func azureIsAIFoundryAccountManagementPath(segments []string) bool {
	...
	for idx := 2; idx+1 < len(segments); idx++ {
		if strings.EqualFold(segments[idx], "providers") && strings.EqualFold(segments[idx+1], "Microsoft.CognitiveServices") {
			return true
		}
	}
	return false
...
func azureAIFoundryAccountManagementAllowedMethod(method string) bool {
	switch method {
