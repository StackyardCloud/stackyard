...
func TestAzureBatchAvatarRoutesReturnFoundationSuccess(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:   "create avatar batch synthesis",
			method: http.MethodPut,
			path:   "/azure/batchavatar/2024-08-01/batchsyntheses/synth-1?api-version=2024-08-01",
			body: []byte(`{
				"displayName":"stackyard-batch-avatar",
				"description":"local emulation",
				"inputKind":"PlainText",
				"inputs":[{"content":"Hello from Stackyard avatar"}]
			}`),
		},
		{
			name:   "get avatar batch synthesis",
			method: http.MethodGet,
			path:   "/azure/batchavatar/2024-08-01/batchsyntheses/synth-1?api-version=2024-08-01",
		},
		{
			name:   "list avatar batch syntheses",
			method: http.MethodGet,
			path:   "/azure/batchavatar/2024-08-01/batchsyntheses?api-version=2024-08-01",
		},
		{
			name:   "delete avatar batch synthesis",
		...
		},
		{
			...
			method: http.MethodGet,
			path:   "/azure/batchavatar/2024-08-01/operations/op-123?api-version=2024-08-01",
		},
	}
...
func TestAzureBatchAvatarInvalidAPIVersionReturnsBadRequest(t *testing.T) {
	t.Parallel()
	...
	path := "/azure/batchavatar/2024-08-01/batchsyntheses?api-version="
	...
	if payload["provider"] != providerAzure || payload["path"] != "/azure/batchavatar/2024-08-01/batchsyntheses" {
		t.Fatalf("unexpected payload for invalid api-version: %#v", payload)
	}
}

func TestAzureBatchAvatarUnknownNestedRouteReturnsFoundationSuccess(t *testing.T) {
	t.Parallel()
	...
	path := "/azure/batchavatar/2024-08-01/unknown-resource"
	resp := providerContractRequest(t, ts, http.MethodGet, path, nil, map[string]string{
		"Authorization":             "SharedKey devstoreaccount1:signature",
		"Ocp-Apim-Subscription-Key": "stackyard-local-subscription-key",
	})
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 for unknown batch avatar nested route, got %d body=%s", resp.StatusCode, string(providerContractBody(t, resp)))
	}
	payload := providerContractJSONMap(t, resp)
