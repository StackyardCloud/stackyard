...
const azureSearchManagementPrivateEndpointConnectionsSubscriptionsBase = "/azure/subscriptions/"

func (s *Server) handleAzureSearchManagementResourceManagerPrivateEndpointConnectionsRouter(w http.ResponseWriter, r *http.Request) bool {
	path := rawRequestPath(r)
	if !strings.HasPrefix(path, azureSearchManagementPrivateEndpointConnectionsSubscriptionsBase) {
		return false
	}
	...
	if !strings.EqualFold(segments[0], "subscriptions") ||
		...
		segments[7] == "" ||
		segments[8] != "privateEndpointConnections" {
		return false
	}
