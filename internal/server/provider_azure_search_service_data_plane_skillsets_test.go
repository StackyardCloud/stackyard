...
func TestAzureSearchServiceDataPlaneSkillsetsRoutesReturnNotImplemented(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			path:   "/azure/skillsets?api-version=2025-09-01",
			body: []byte(`{
				"name":"hotels-skillset",
				"skills":[
					{"@odata.type":"#Microsoft.Skills.Text.LanguageDetectionSkill","name":"lang","context":"/document","defaultLanguageCode":"en"}
				]
			}`),
		},
		{
			...
			path:   "/azure/skillsets('hotels-skillset')?api-version=2025-09-01",
			body: []byte(`{
				"name":"hotels-skillset",
				"skills":[
					{"@odata.type":"#Microsoft.Skills.Text.LanguageDetectionSkill","name":"lang","context":"/document","defaultLanguageCode":"en"}
		...
		},
		{
			name:   "list",
			method: http.MethodGet,
			path:   "/azure/skillsets?api-version=2025-09-01&$select=name",
		},
	}
...
func TestAzureSearchServiceDataPlaneSkillsetsUnsupportedNestedPathReturnsNotImplemented(t *testing.T) {
	t.Parallel()
	...
	path := "/azure/skillsets('hotels-skillset')/unsupported/segment"
	resp := providerContractRequest(t, ts, http.MethodGet, path, nil, map[string]string{
		"Authorization": "SharedKey devstoreaccount1:signature",
	})
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("expected 200 for unknown skillsets nested path, got %d body=%s", resp.StatusCode, string(providerContractBody(t, resp)))
	}
	payload := providerContractJSONMap(t, resp)
